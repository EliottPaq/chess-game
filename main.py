...
def print_board () :
    """
    this function just print the board (white and black square)
    """    
    ...
    for y in axis_value :
        for x in axis_value :
            if color_value == 0 :
                ...
                if x != 525 :# at the last rectangle we are not changing the color because otherwise it will wouldn't be a real chess board
                    color_value = 1
            elif color_value== 1 :
                pygame.draw.rect(screen,"black",[x,y,75,75]) #75,75 is the height and length of each square
                if x != 525:
                    color_value = 0# at the last rectangle we are not changing the color because otherwise it wouldn't be a real chess board
...
def print_piece (grid:list) :
    ...
    for row in grid:
        for piece in row :
            if piece != None:
                piece_surf = piece.sprite.get_rect(center = (axis_value[piece.position[1]]+37.5,axis_value[piece.position[0]]+37.5)) # 37.5 is the half of 75 by using this value we are placing our piece in the middle of each square
...
def screen_update(board:list,movement_list:list):
    ...
    Args:
        ...
        movement_list (list): a list of move possible
    """
...
def move_piece(piece:object,board:list,position:list,list_of_move:list) :
    ...
    if board[position[0]][position[1]] != None : #if we aren't moving to an empty squares
        if board[position[0]][position[1]].color == piece.color : #if the piece is going to a piece the same color it can only be a rock
            if piece.color == "B" :
        ...
        movement_soud.play()
        if piece.name == "P" and (position[1]-1 == piece.position[1] or position[1]+1 == piece.position[1]) : #if the piece is doing on a "en passant"
            if piece.color == "W":
                board[position[0]+1][position[1]] = None #delete the piece who is getting capture by the "en passant"
            else:
                board[position[0]-1][position[1]] = None #delete the piece who is getting capture by the "en passant"
        board[position[0]][position[1]] = piece
        board[piece.position[0]][piece.position[1]] = None
...
def clone_board (board:list,second_board:list)-> None :
    ...
    Args:
        ...
        second_board (list): the grid who is getting copied on
    """
    ...
    for y in range(len(board)):
        for x in range(len(board[y])):
            if board[y][x] == None:  #if the square is empty we are chaning to None
                ...
                second_board[y][x] = Pawn(board[y][x].color,board[y][x].position,board[y][x].sprite_pathing,board[y][x].move_counter) #calling the object witht the current parameters
...
def check(position:list,board:list,piece_selected:object,turn:str,list_of_move:list)->bool:
    """
    return True if there no check with the move we are planning to do
    Args:
                    ...
                    if piece.color == "W":
                        for move_of_piece in piece.movement_list:
                            if board[move_of_piece[0]][move_of_piece[1]] != None and board[move_of_piece[0]][move_of_piece[1]].name == "K" and board[move_of_piece[0]][move_of_piece[1]].color == "B" :
                                return True
    return False #if the program go to this it mean no piece can eat the king so there is no check
...
def checkmate (board:list,second_board:list,turn:str,list_of_move:list)->bool:
    """
        ...
        A checkmate respect two condition :-the king can't go anwere without being eaten 
                                           -no piece can go between or on the piece who check and the king
                                           -if the king can't go anywere without being eaten and there 2 piece or more checking him it's a checkmate
    Args:
        board (list): the board
        second_board (list): a second board who is the copy of the first one
        turn (str): who's turn
    ...
    good_color_piece = [] #stocking all the piece who could stop the check
    if turn == "W":
                    ...
                    if board[y][x] != None:
                        if board[y][x].name == "K" and board[y][x].color == "B" : # if the piece is the king we are looking for
                            for move in board[y][x].movement_list:
                                if check(move,board,board[y][x],"B",list_of_move): # if the king can escape somwhere without being eat it's not a checkmate
                            ...
                            for move in board[y][x].movement_list:
                                if board[move[0]][move[1]] != None:
                                    if board[move[0]][move[1]].name == "K" and board[move[0]][move[1]].color == "B" :
                                        piece_who_check.append(board[y][x]) # if the can be eatean by this we adding it to the correct list
                        else:
                            good_color_piece.append(board[y][x]) #if the piece is on the same team as the king we add it to the list of all the piece who could stop the checkmate
            if len(piece_who_check) > 1 : #if the king have nowhere to go and there is more than piece checking him it's checkmate
                return True
            else:
                for piece in good_color_piece:
                    for move_can_do in piece.movement_list : #for all the move of all the friendly piece
            ...
            if piece != None :
                if piece.color != turn : # if the piece of the next team
                    for move in piece.movement_list :
                        if check(move,board,piece,"B",list_of_move): # if we can go somewhere whithout getting check then it's not a pat
                            list_of_move.pop(-1) #deleting the last element because in the call of "check" we are adding the move we are trying
                            clone_board(second_board,board) #reseting the board
                            return False
                        list_of_move.pop(-1)
...
def no_piece_to_mate(board:list)->bool:
    """
    the null by having not having enough piece is when we have the current situation : king vs king or minor piece + king vs king
    Args:
        board (list): the board
